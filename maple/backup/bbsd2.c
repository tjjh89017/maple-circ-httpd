  ...
  int mode;
{
  ...
  if (!cuser.userlevel)         /* guest */
  {
    ...
    #define GUEST_NICK_NUM 5
    char nick[GUEST_NICK_NUM][5] = {"過客", "訪客", "沙塵", "遊者", "路人"};
    sprintf(cuser.username, "育才街上的%s", nick[rand() % GUEST_NICK_NUM]);
  }
...
#  ifdef GUEST_WHERE
  if (!cuser.userlevel)		/* guest */
  {
    #define GUEST_WHERE_NUM 10
    char from[GUEST_WHERE_NUM][15] = {"一宿前販賣機", "體育大樓後草地", "校園東南隅", "入德之門", "慎思樓遺址", "樟園","二宿旁販賣機", "敬業樓福利社", "麗澤樓福利社", "光中亭"};
    strcpy(utmp.from, from[rand() % GUEST_WHERE_NUM]);
  }
  else
#  endif	/* GUEST_WHERE */
  {
  ...
  char *content;
{
  ...
  /* floatJ: 改成從Announce裡面調用FILM_LOGINMSG來顯示 */
#if 1
  outs(" \033[1;31m★\033[m 請以\ \033[1;36m" STR_GUEST "\033[m 參觀，或以 \033[1;35m" STR_NEW "\033[m 註冊。");
#else
...
test2:
 if (!vget(b_lines - 2, 40, "[您的密碼] ", passbuf, PSWDLEN + 1, NOECHO))
	...
	if (!str_cmp(cuser.userid, str_sysop))
	{
	  ...
	  {
	     vmsg("就算您把define打開也沒有用，因為su程式碼被拔掉了");	  
	  }
	  else
#endif
	  {
	...
	if(!HAS_PERM(PERM_ALLADMIN))
	{
	  ...
	  if (!HAS_PERM(PERM_ALLADMIN) && (multi = utmp_count(cuser.userno, 0)) >= MULTI_MAX || 	/* 線上已有 MULTI_MAX 隻自己，禁止登入 */
	    (!multi && acct_load(&cuser, uid) < 0)){			/* yiting.050101: 若剛已踢掉所有 multi-login，那麼重新讀取以套用變更 */
	    vmsg("抱歉，您重複登入的分身過多，無法登入‧‧‧");
 	    login_abort("\n登入人數過多，無法登入 ...");
	  }
	}
	else
	{
	  ...
	  if (pid && vans("要踢掉重複登入的帳號嗎？(Y/N) [N] ") == 'y' && pid == ui->pid)
          {
	    if ((kill(pid, SIGTERM) == -1) && (errno == ESRCH))
	      utmp_free(ui);
	    else
	...
	login_abort("\nguest登入人數過多，無法登入......");
      ...
      logattempt(' ', content);
      cuser.userlevel = 0;	/* Thor.981207: 強制寫回cuser.userlevel */
...
tn_main()
{
  ...
  film_out((ap_start % 3) + FILM_OPENING0, 0 );	/* 亂數顯開頭畫面 */
...
if(port==0){
FILE *fp;
fp = fopen("/home/bbs/test_t.txt","a");
fprintf(fp,"0\n");
fclose(fp);
}
...
if(argc==1){
FILE *fp;
fp = fopen("/home/bbs/test_t.txt","w");
fprintf(fp,"Test Start.\n");
fclose(fp);
}
