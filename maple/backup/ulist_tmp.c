...
extern UCACHE *ushm;
extern XZ xz[];
int idlemode = 1; /*dust: 1:Normal mode, 0:idle-only mode */
  ...
  int sysop;
{
  ...
  if (idlemode) {
    ...
    /* dust.080422: 這行不是原本的程式...... */
//    sprintf(noStr, "%6d", num);  //直接以%6d加上色碼會爆炸，所以折衷...
    sprintf(noStr, "  \033[1;30m%4d\033[m", num);  //超過10000人記得改掉這裡。

    if (up->userlevel & PERM_LABEL)
    {
      sprintf(buf, " %s", up->label);
    }
    else if (up->userlevel & PERM_ALLADMIN)
    {
      if(up->status & STATUS_BIRTHDAY){
        strcpy(buf, " \033[1;31m站誤\033[m");
      }else{
        strcpy(buf, " \033[1;32m站誤\033[m");
      }
    }
...
{   /* floatJ: 新增 麻吉 */
  ...
  if (is_mymate(userno))
    ...
    strcpy(noStr, "    \033[1;33m★\033[m");

      /* 小心星號的幻視現象 */

  else

    sprintf(noStr, "  \033[1;30m%4d\033[m", num);
  ...
  if (strlen(buf) == 0)
  {
    ...
    if ((diff = now - up->idle_time) >= 60){    /* 超過 60 秒才算閒置 */
      if(diff>=3600)                /* 閒置超過 1hr */
        sprintf(buf, "%2dh%02dm", diff / 3600, (diff % 3600) / 60);
      else
        sprintf(buf, "%2dm%02ds", diff / 60, diff % 60);
    }
#else
...
#  endif
    ...
    up->status & STATUS_BIRTHDAY ? "\033[1;5;35m" :
  ...
  //prints("%6d%c%c%s%-13s%-*.*s\033[m%-*.*s%-11.10s%s\n",
    //num,ufo & UFO_SUPERCLOAK ? '>' : ufo & UFO_CLOAK ? ')' : ' ', pager,


/* floatJ: 把原來的編號改成兩用*/
prints("%s%c%c%s%-13s%-*.*s\033[m%-*.*s%-11.10s%s\n", 
noStr, ufo & UFO_SUPERCLOAK ? '>' : ufo & UFO_CLOAK ? ')' : ' ', pager,
  ...
  XO *xo;
{
  ...
  if (!idlemode) prints(NECKER_ULIST, d_cols >> 1, "", d_cols - (d_cols >> 1) + 4, pickup_ship ? "友誼" : "故鄉");
  else prints(NECKER_ULIST_LABEL, d_cols >> 1, "", d_cols - (d_cols >> 1) + 4, pickup_ship ? "友誼" : "故鄉");
  ...
  XO *xo;
{
  if (HAS_PERM(PERM_ALLACCT))
  {
    ...
    if (pid = up->pid)
    {
      sprintf(buf, "警告：現在即將踢 %s 下線，請您確認？ (Y/N) [N] ", up->userid);
  ...
  XO *xo;
{
  ...
  /* floatJ: 極光從此全站開放隱身權限，但是 guest 不能隱身 */
...
static KeyFunc ulist_cb[] =
{
  ...
  'S', ulist_ship,
  'p', ulist_pager,
  'o', ulist_rcver,     /* floatJ.040824: 廣播天線切換快速鍵*/
  'I', ulist_lock,      /* qazq.030719: 使用者選單直接閒置 */
  'x', ulist_idle,      /*dust: 切換顯示模式  */
