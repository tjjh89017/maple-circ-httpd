...
//	vmsg("etc/npc has been open for r!"); /*debug*/
{
...
//	 vmsg("BMW new!"); /*debug*/
         ...
         bmw_npcAI(new.msg);
//	 vmsg("bmw_npcAI has got message!LAST!!"); /*debug*/
	 //delay(25000);
         bmw_send(mptr->caller, &new);
//	 vmsg("bmw_sended!"); /*debug*/
         usr_fpath(fpath, cuser.userid, fn_bmw);
//	 vmsg("usr_fpath!"); /*debug*/
         rec_add(fpath, &new, sizeof(BMW));
//	 vmsg("rec_added!"); /*debug*/
         usr_fpath(fpath, cuser.userid, fn_amw);
//	 vmsg("usr_fpathed!"); /*debug*/
         if (fp = fopen(fpath, "a"))
         {
//	 vmsg("fopen a ed!"); /*debug*/
           fprintf(fp, BMW_FORMAT2 " %s\n",
             new.userid, new.msg, Btime(&new.btime));
/* floatJ.070917: NPC這個patch下面的這行fclose要拿掉，否則會卡死*/
       //    fclose(fp);
	//	npc_jumpclose = 1;
//	 vmsg("fclosed! FINAL"); /*debug*/
         }
// vmsg("out 1"); /*debug*/
       }
// vmsg("out 2"); /*debug*/
